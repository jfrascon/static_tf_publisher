...
def _walk_data_files(source_root: str, install_root: str) -> list[tuple[str, Sequence[str]]]:
    ...
    for current_root, _, filenames in os.walk(source_root):
        ...
        install_dir = (
            install_root if relative_dir == '.' else os.path.join(install_root, relative_dir)
        )
        files: Sequence[str] = [
            os.path.join(current_root, filename) for filename in sorted(filenames)
        ]
...
data_files += _walk_data_files('docs', f'share/{package_name}/docs')
...
setup(
    ...
    entry_points={
        'console_scripts': [
            'static_tf_publisher_node = static_tf_publisher.static_tf_publisher_node:main'
        ]
    },
